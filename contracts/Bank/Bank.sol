...
contract Bank is Ownable, IERC721Receiver, Pauseable {
    ...
    event TokenStaked(address owner, uint256 tokenId, uint256 timestamp);
    event TokenClaimed(
        address owner,
        uint256 tokenId,
        uint256 timestamp,
    ...
    function getUserStaked(address _addr)
        public
        view
        returns (uint256[] memory)
    {
        return userStaked[_addr];
    }
